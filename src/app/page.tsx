    ...
    <main className='flex min-h-screen flex-col items-center justify-between p-8 font-barlow font-semibold'>
      <header className='border-2 border-header-outline rounded-lg max-w-[85vw] w-[650px] flex justify-between flex-row p-2 sm:p-4 uppercase'>
        <div className='max-sm:text-xs text-white font-bold space-x-0 flex flex-col justify-center ml-2'>
          <p className='h-2.5 sm:h-3.5 leading-3'>Rock</p>
          <p className='h-2.5 sm:h-3.5 leading-3'>Paper</p>
          <p className='h-2.5 sm:h-3.5 leading-3'>Scissors</p>
          <p className='h-2.5 sm:h-3.5 leading-3'>Lizard</p>
          <p className='h-2.5 sm:h-3.5 leading-3'>Spock</p>
        </div>
        <div className='bg-white rounded-md text-[hsl(229, 25%, 31%)] text-center min-h-full w-20 sm:w-24 shadow-sm p-2'>
          <p className='text-score text-xs sm:text-sm tracking-widest'>Score</p>
          <h2 className='text-points text-4xl sm:text-5xl font-bold'>12</h2>
        </div>
      </header>

      <section className='h-[22rem] sm:h-[36rem] flex flex-col justify-center relative'>
        <Image
          className='absolute text-points stroke-2 w-64 h-64 right-8 sm:w-[23rem] sm:h-[23rem] sm:right-20'
          src={'/images/bg-pentagon.svg'}
          alt='Pentagon Image'
          width={48}
          height={48}
        />
        <div className='flex justify-center items-center w-80 sm:w-[32rem] h-24 z-10'>
          <TokenCircle token='scissors' tokenColor='from-scissors to-scissors-end' />
        </div>
        <div className='flex flex-row justify-between w-80 h-28 sm:w-[32rem] sm:h-52 z-10'>
          <TokenCircle token='spock' tokenColor='from-spock to-spock-end' />
          <TokenCircle token='paper' tokenColor='from-paper to-paper-end' />
        </div>
        <div className='flex flex-row justify-center gap-6 sm:justify-around items-center w-80 sm:w-[32rem] h-36 z-10'>
